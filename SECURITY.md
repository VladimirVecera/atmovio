...
SkyWatch runs with root privileges on the Raspberry Pi (it manages Docker, disks and WireGuard) and
serves its UI over plain HTTP. It is designed for a **trusted home network or a VPN**, not for the open internet.

## Do

- keep port 80 (SkyWatch), 8971 (Frigate), 9090 (Cockpit) and 9443 (Portainer) reachable only from your LAN/VPN;
...
- port-forward SkyWatch or Frigate to the internet;
...
released as a new `update-skywatch.sh`.
