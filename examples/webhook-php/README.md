# Atmovio webhook receiver (PHP example)

A tiny, dependency-free receiver you can drop on any PHP hosting (PHP 8.0+). Atmovio sends
...
1. Upload `webhook.php`, `index.php` and `config.php` to a folder on your web, e.g. `https://example.com/atmovio/`.
2. Edit `config.php`:
   - `ATMOVIO_TOKEN` – a long random secret (`openssl rand -hex 32`).
   - optionally `ATMOVIO_MAIL_TO` to forward alerts by e-mail, and `ATMOVIO_VIEW_PASSWORD` to protect the page.
3. Make sure PHP can write to `data/` (it is created automatically; on Apache a `.htaccess` denies direct access to it).
4. In Atmovio open **Nastavení → Upozornění → Vlastní web (webhook)**, paste the URL
   `https://example.com/atmovio/webhook.php` and the same token, click **Uložit a otestovat spojení**.
