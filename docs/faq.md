...
**Can I use an SD card only?** Yes for the system; recordings need the USB HDD (SD cards die from constant writes). Without a disk SkyWatch runs in live-view mode.

**Does it detect people/cars?** No – Frigate object detection is intentionally off (no accelerator on the Pi). SkyWatch is about the sky. You can enable detection in Frigate yourself, at the cost of CPU.
...
**Free AI limits?** Google Gemini Flash-Lite free tier is a few hundred requests/day; SkyWatch stops at your configured daily limit and skips unchanged/dark frames.

**How do I back up?** `/opt/nvr/skywatch/config.json` (settings), `/opt/nvr/frigate/config/config.yml` (cameras), `/etc/wireguard/wg-remote.conf` (VPN). Recordings are on the HDD.

**Update went wrong?** The updater rolls back automatically; backups are in `/opt/nvr/skywatch/backups/`. Manual restore: copy `app.py` from a backup and `systemctl restart skywatch`.

**Where are the logs?** Page **Logy** (download button), or `journalctl -u skywatch -n 200`.
