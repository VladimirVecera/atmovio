...
**Stačí SD karta?** Pro systém ano; záznamy potřebují USB HDD (SD karty od stálého zápisu umírají). Bez disku běží Atmovio v režimu živého náhledu.

**Detekuje lidi/auta?** Ne – detekce objektů Frigate je záměrně vypnutá (na Pi není akcelerátor). Atmovio je o obloze. Ve Frigate si ji můžeš zapnout sám, za cenu CPU.
...
**Limity AI zdarma?** Google Gemini Flash-Lite zdarma je pár set dotazů denně; Atmovio se zastaví na nastaveném denním limitu a přeskakuje nezměněné/tmavé snímky.

**Jak zálohovat?** `/opt/nvr/atmovio/config.json` (nastavení), `/opt/nvr/frigate/config/config.yml` (kamery), `/etc/wireguard/wg-remote.conf` (VPN). Záznamy jsou na HDD.

**Aktualizace se nepovedla?** Aktualizátor se vrátí sám; zálohy jsou v `/opt/nvr/atmovio/backups/`. Ruční obnova: zkopíruj `app.py` ze zálohy a `systemctl restart atmovio`.

**Kde jsou logy?** Stránka **Logy** (tlačítko Stáhnout), nebo `journalctl -u atmovio -n 200`.
